...
class Samples():
    ...
    def seek_parts(self, x: int, y: int) -> list:

        log.debug('checking for parts around %d:%d...', x, y)
...
for row in input_data:
    ...
    for value in re.finditer('[\#|\$|\%|\&|\*|\+|\-|\/|\=|\@]', row.strip()):
        new_parts = samples.seek_parts(value.start(), current_row)
