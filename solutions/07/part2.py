...
class Hand():
    ...
    def _calculate(self):
        ...
        # Let's process the hand for any JOKER upgrades!

        if self.JOKER in counts:
            if self.combo != HandCombos.FIVE_OF_A_KIND:
                self.original_combo = self.combo

                if self.combo == HandCombos.FOUR_OF_A_KIND:
                    self.combo = HandCombos.FIVE_OF_A_KIND
                elif self.combo == HandCombos.FULL_HOUSE:
                    self.combo = HandCombos.FIVE_OF_A_KIND
                elif self.combo == HandCombos.THREE_OF_A_KIND:
                    self.combo = HandCombos.FOUR_OF_A_KIND
                elif self.combo == HandCombos.TWO_PAIR:
                    if counts[self.JOKER] == 2:
                        self.combo = HandCombos.FOUR_OF_A_KIND
                    else:
                        self.combo = HandCombos.FULL_HOUSE
                elif self.combo == HandCombos.PAIR:
                    self.combo = HandCombos.THREE_OF_A_KIND
                else:
                    self.combo = HandCombos.PAIR

    def dump(self) -> str:
        return f'Hand: Cards:{self.cards}, Bid:{self.bid}, Combo:{self.combo}, OC:{self.original_combo}'
